package chapter04.ex07;

public class Break_Control_Keyword {
	public static void main(String[] args) {
		/*
		 * break 제어 키원드 : 제어문 (if , switch, for, while, do while)
		 * 	-제어문을 탈출 할떄 사용됨
		 * 1. 단일 loop문에서 break 탈출
		 * 
		 * 
		 */
		
		for ( int i = 0; i < 10; i++){
			  System.out.println(i);
			  //break;
			  if(i==5) {		//if 조건을 사용해서 조건이 만족 되었을떄 탈출
				  break;
			  }
		 }
		// 0, 1, 2, 3, 4, 5
		
		/*
		 * 2. 다중(이중) loop문에서 break탈출
		 * 	case1: 하나의 반복문만 탈출하는 경우
		 */
		for(int i = 0; i < 5; i++) {
			for(int j = 0; j < 5 ; j++) {
				if(j==2) {
					break;
				}
				System.out.println(i + " , " + j);
			}
			System.out.println();
		}
		
		
		System.out.println("========LABLE을 사용해서 한꺼번에 LABLE 위치로 점프=====================================");
		/*
		 * case2 : 점프해서 한번에 바깥쪽의 for 문을 탈출 ( LABEL)
		 */
		pos1: for(int i = 0; i < 5 ; i++) {
			for(int j= 0; j < 5 ; j++) {
				if (j ==2) {
					break pos1;	//레이블(LABLE로 지정된 곳으로 점프해서 탈출
				}
				System.out.println(i + " , " + j);
			}
			System.out.println();
		}
		/*
		 * case3: 점프해서 한번에 바깥쪽의 for 문을 탈출 (변수 값을 조정해서 탈출)
		 */
		for(int i = 0; i < 5 ; i++) {
			for(int j= 0; j < 5 ; j++) {
				if (j ==2) {
					i = 100; //바깥쪽 for문의 변수의 값을 큰값을 넣어서 조건을 false
					break;
				}
				System.out.println(i + " , " + j);
			}
			System.out.println();
		}
	}
}








