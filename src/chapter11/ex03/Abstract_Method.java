...
abstract class Animal{		//클래스 내부에 추상 메소드가 1개라도 존재하면, 추상 클래스가 되어야 한다 .
	...
	//프로젝트 설계시 메소드 다른 이름으로 사용하지 못하도록 할수 있다
	// 부모 클래스에서는 메소드 이름을 선언만 해주고, 자식 클래스에서 구현하도록 함 
	// 자식 클래스에서는 메소드 이름을 변경할수 없다. 
	abstract void run();  //구현부가 없는 선언만된 메소드 : 추상 메소드
...
class Cat extends Animal{
	...
	// 부모 클래스의 추상 메소드를 구현하지 않는 경우는 자신도 추상 클래스가 되어야 한다. 
